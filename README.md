# Awesome-Voiceprint
A curated list of awesome Voiceprint Recognition papers.

# Speaker Recognition

## 2021
* [Attention Back-end for Automatic Speaker Verification with Multiple  Enrollment Utterances](https://arxiv.org/abs/2104.01541)
* [Out of a hundred trials, how many errors does your speaker verifier  make?](https://arxiv.org/abs/2104.00732)
* [Unit selection synthesis based data augmentation for fixed phrase speaker verification](https://arxiv.org/abs/2102.09817)
* [Learnable MFCCs for Speaker Verification](https://arxiv.org/abs/2102.10322)
* [Triplet loss based embeddings for forensic speaker identification in Spanish](https://arxiv.org/abs/2102.12564)
* [Target Speaker Verification with Selective Auditory Attention for Single  and Multi-talker Speech](https://arxiv.org/abs/2103.16269)

## 2020
* [ECAPA-TDNN: Emphasized Channel Attention, Propagation and Aggregation in TDNN Based Speaker Verification](https://arxiv.org/abs/2005.07143) | [code](https://github.com/speechbrain/speechbrain)
* [In defence of metric learning for speaker recognition](https://arxiv.org/abs/2003.11982) | [code](https://github.com/clovaai/voxceleb_trainer)
* [Augmentation adversarial training for unsupervised speaker recognition](https://arxiv.org/abs/2007.12085)
* [Double Multi-Head Attention for Speaker Verification](https://arxiv.org/abs/2007.13199)
* [UIAI System for Short-Duration Speaker Verification Challenge 2020](https://arxiv.org/abs/2007.13118)
* [Self Attentive Multi Layer Aggregation with Feature Recalibration and Normalization for End to End Speaker Verification System](https://arxiv.org/abs/2007.13325)
* [Probabilistic embeddings for speaker diarization](https://arxiv.org/abs/2004.04096)
* [Deep Normalization for Speaker Vectors](https://arxiv.org/abs/2004.04095)
* [Bayesian x-vector: Bayesian Neural Network based x-vector System for Speaker Verification](https://arxiv.org/abs/2004.04014)
* [Learning to fool the speaker recognition](https://arxiv.org/abs/2004.03434)
* [Universal Adversarial Perturbations Generative Network for Speaker  Recognition](https://arxiv.org/abs/2004.03428)
* [Multi-Scale Aggregation Using Feature Pyramid Module for  Text-Independent Speaker Verification](https://arxiv.org/abs/2004.03194)
* [AM-MobileNet1D: A Portable Model for Speaker Recognition](https://arxiv.org/abs/2004.00132)
* [End-to-End Neural Diarization: Reformulating Speaker Diarization as Simple Multi-label Classification](https://arxiv.org/abs/2003.02966)
* [NPLDA: A Deep Neural PLDA Model for Speaker Verification](https://arxiv.org/abs/2002.03562)

## 2019
* [Utterance-level Aggregation For Speaker Recognition In The Wild](http://www.robots.ox.ac.uk/~vgg/publications/2019/Xie19a/xie19a.pdf)
 
## 2018
* [GENERALIZED END-TO-END LOSS FOR SPEAKER VERIFICATION](https://arxiv.org/abs/1710.10467v3)

# Others
* [Multichannel CRNN for Speaker Counting: an Analysis of Performance](https://arxiv.org/abs/2101.01977)
* [End-to-End Speaker Height and age estimation using Attention Mechanism with LSTM-RNN](https://arxiv.org/abs/2101.05056)
* [SoundCLR: Contrastive Learning of Representations For Improved  Environmental Sound Classification](https://arxiv.org/abs/2103.01929)
* [Environmental Sound Classification on the Edge: Deep Acoustic Networks  for Extremely Resource-Constrained Devices](https://arxiv.org/abs/2103.03483)
* [GISE-51: A scalable isolated sound events dataset](https://arxiv.org/abs/2103.12306)
* [Guided Training: A Simple Method for Single-channel Speaker Separation](https://arxiv.org/abs/2103.14330)
* [SubSpectral Normalization for Neural Audio Data Processing](https://arxiv.org/abs/2103.13620)
* [SpecAugment++: A Hidden Space Data Augmentation Method for Acoustic  Scene Classification](https://arxiv.org/abs/2103.16858)
* [MixSpeech: Data Augmentation for Low-resource Automatic Speech Recognition](https://arxiv.org/abs/2102.12664)
* [Fundamental Frequency Feature Normalization and Data Augmentation for Child Speech Recognition](https://arxiv.org/abs/2102.09106)
